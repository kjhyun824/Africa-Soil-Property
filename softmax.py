...
parser.add_argument('--index', default = 5000, help='index', type=int)
...
for i in range(0,len(test_set[0])-1): # column index 
    for j in range(0,len(test_set)-1): # data index
        test_set[j][i] = float(test_set[j][i])
        test_set[j][i] = (test_set[j][i] - mininput[i]) / (maxinput[i] - mininput[i])
...
#logits = tf.contrib.layers.fully_connected(inputs=X, num_outputs = 5, scope='fc')
